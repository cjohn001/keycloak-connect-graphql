...
export class KeycloakContextBase implements AuthContextProvider {
    ...
    constructor(token?: Keycloak.Token, keycloak?: Keycloak.Keycloak, authorizationConfiguration?: AuthorizationConfiguration) {
    ...
    public hasRole(role: string): boolean {
        return this.isAuthenticated() && (this.accessToken?.hasRole?.(role) ?? false);
    }
    ...
    public async hasPermission(expectedPermissions: string | string[]): Promise<boolean> {
...
export class KeycloakContext extends KeycloakContextBase implements AuthContextProvider {
    ...
    constructor({req}: { req: GrantedRequest }, keycloak?: Keycloak.Keycloak, authorizationConfiguration?: AuthorizationConfiguration) {
        const token = (req && req[CONTEXT_KEY] && req[CONTEXT_KEY]?.grant) ? req[CONTEXT_KEY].grant.access_token : undefined;
